    ...
    const c10::optional<Tensor>& val_opt, const Tensor& x)
{
    ...
    TORCH_CHECK(N64 <= static_cast<int64_t>(std::numeric_limits<int>::max()), "N exceeds kernel index range");
    TORCH_CHECK(F64 <= static_cast<int64_t>(std::numeric_limits<int>::max()), "F exceeds kernel index range");
    TORCH_CHECK(col.numel() <= static_cast<int64_t>(std::numeric_limits<int>::max()),
                "nnz exceeds kernel index range");
    ...
    if (val_opt.has_value() && val_opt->defined()) {
        ...
        TORCH_CHECK(val_opt->numel() == col.numel(), "val must contain one weight per nonzero");
    ...
    if (const char* env = std::getenv("AUTOSAGE_VEC4")) {
        ...
        want_vec4 = !(value == "0" || value == "false" || value == "no" || value == "off");
    }
