...
// One warp handles one row from rows[]. Lanes are feature-parallel.
// VEC4 path: each lane owns 4 contiguous features starting at lane*4 within the tile.
// For FTILE ∈ {64,128}, this needs at most one iteration per lane.
...
__global__ void spmm_csr_rows_kernel(
    ...
    const int32_t* __restrict__ rows,   // [R]
    int R, int F)
{
...
// -------------- Public launcher called by split --------------
